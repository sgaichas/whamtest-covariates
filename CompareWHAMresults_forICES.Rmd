...
df.aic.tmp <- as.data.frame(wham::compare_wham_models(mods2, table.opts=list(sort=FALSE, calc.rho=T))$tab)
...
```{r}
basediag <- as.data.frame(wham::compare_wham_models(mods[1], table.opts=list(sort=FALSE, calc.rho=T))$tab)
...
We can see the WHAM comparison outputs of the ecov models by runnning this

```{r}
wham::compare_wham_models(mods2)
```
